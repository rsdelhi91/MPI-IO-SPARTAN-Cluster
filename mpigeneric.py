...
# order to specify the cores we set the number 1 to 4 (for 4 cores) or 8 (for 
# 8 cores) 
#
#-------------------------------------------------------------------
...
# If we are at the Master (Rank = 0) then start aggregating the results count for the
# search_word and print it out.
if rank == 0:
...
# If we are at the Master (Rank = 0) then flatten the lists of lists gathered from all
# the cores and decode them (as they are in Byte Array), following which include them
# in a dictionary if that tweeter was not present previously and set its value to 1, else
# increase the count for that tweeter by 1.
if rank == 0:
	if size > 2:
...
# If we are at the Master (Rank = 0) then flatten the list of lists gathered from all
# the cores and decode them (as they are in Byte Array), following which include them
# in a dictionary if that topic was not present previously and set its value to 1, else 
# increase the count for that topic by 1
if rank == 0:
	if size > 2:
...
#-------------------------------------------------------------------
# Finally, sort the dictionary created in descending order and limit the list by 10 using
# list splicing to display the top 10 tweeters and top 10 topics present in the large tweet
# file given to us.
if rank == 0:
